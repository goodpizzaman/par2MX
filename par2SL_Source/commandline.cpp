        ...
        while (fn != filenames->end())
        {
          ...
          if (parfilename.length() == 0)
          {
            ...
            if (operation != opCreate)
            {
              ...
              if (version == verUnknown)
              {
				  cerr << "The recovery file does not exist: " << DiskFile::FS2UTF8(filename) << endl;
                return false;
              }
            }
            else
          ...
          else
          {
            ...
            if (!DiskFile::FileExists(filename))
            {
				cout << "Ignoring non-existent source file: " << DiskFile::FS2UTF8(filename) << endl;
            }
			else
			{
				...
				{
				  cout << "Skipping 0 byte file: " << DiskFile::FS2UTF8(filename) << endl;
				}
			} //end file exists
          }
