...
bool Par1Repairer::LoadRecoveryFile(string filename)
{
  ...
  // Open the file. Note we specify false for the tryToCache argument, so the behavior is as in the
  // original version of the program. The focus in the program is on par2, not par1.
  if (!diskfile->Open(filename, false))
  {
...
bool Par1Repairer::ComputeRSmatrix(void)
{
  ...
  while (sourceiterator != sourcefiles.end())
  {
    ...
    if (sourceblock->IsSet())
    {
      ...
      if (!sourceblock->Open(false))	// Note: not sure if false is the most efficient here.
      {
        return false;
      }
  ...
  while (inputblock != inputblocks.end())
  {
    ...
    if (!recoveryblock->Open(false))	// Note: not sure if false is the most efficient here
    {
      return false;
    }
